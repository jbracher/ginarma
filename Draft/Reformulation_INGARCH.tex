...
\end{align}
where $\lambda_0$ and $X_0$ are again fixed. Conditional on the past, $X_t$ follows a compound Poisson distribution with mean $\lambda_t$ under this definition. Note that in \eqref{eq:N_CP_original}, the condition is also on $N_{t - 1}, \dots, N_1$, but we suppressed this to shorten notation. This formulation is somewhat more restrictive than the class discussed in \citep{Goncalves2015} where the parameter $\psi_t$ of the seondary distribution is allowed to depend on the past of the process. It nonetheless contains a number of well-known models \cite[Observation 2]{Goncalves2015}:
...
\begin{align}
A_{t, i} & = \begin{cases}
1 & \text{with probability } \beta \ \ \ \ \ \ \ (\rightarrow \text{contribution to } R_{t - 1}) \\
\kappa \star (\psi *_G 1) & \text{with probability } 1 - \beta \ \ (\rightarrow \text{ contribution to } \kappa \star (\psi *_G H_{t - 1})),
\label{eq:Z_t_i}
\end{cases}\\
...
As in Fokianos et al \cite{Fokianos2009}, Proposition 1 from Meitz and Saikkonen \cite{Meitz2008} can then be used to show that geometric ergodicity is inherited by $\{(\juv_t, R_t, H_t, X_t, L_t)\}$. %, see Appendix \ref{appendix:proof_meitz}.
...
\item Given $S_t$, $(R_t, H_t, X_t, L_t)$ is independent of all $\juv_u, R_u, H_u, X_u, L_u, u < t$. It is straightforward to see from equations \eqref{eq:juv_t}--\eqref{eq:L_t} or Figure \ref{fig:ingarch_flowchart} that this is the case.
...
$$
with some suitable initialization (e.g. starting close to the expected values).
...
$$
to the discrete Markov chain $\{S_t\}$. Subsequently one can compute an approximatin of the limiting stationary distribution of $X_t$ as
$$
...
$$
L_t^{(i)} \mid X_{t + i}, X_{t + i - 1}, \dots, X_0 \stackrel{\text{ind}}{\sim} \text{Pois}(\beta^{i - 1}[1 - \beta]\kappa X_t). % ; \ \ L_t^{(i)} \perp L_t^{(j)} \mid X_t, i \neq j
$$
Moreover it is clear that $L_t^{(i)} \perp L_u^{(j)} \mid X_t, X_u$ for $t < u < t + i$. Now consider
\begin{align}
...
\end{align}
where we substituted $H_t$ using equation \eqref{eq:sums}.
We note that in analogy to the above argument, $R_0^{(t)}$ is Poisson distributed with rate $\eta\beta^{t - 1}(1 - \beta)$ and independent of all $X_t$ and $L_t^{(i)}$ up to $t - 1$. Conditioned on $X_{t - 1}, \dots, X_0, \eta$, we thus have that the term $N_t$ introduced in equation \eqref{eq:introduce_N_T} is a sum of independent Poisson random variables. This implies
$$
