...
$$
where the vectors $\mathbf{Z}_{t, j} = (Z_{t, 1, j}, \dots, Z_{t, p, j})$ independently follow an arbitrary offspring distribution on $\mathbb{N}_0^p$ with $\mathbb{E}(\mathbf{Z}_{t, j}) = (\kappa_1, \dots, \kappa_p)$ and finite variances $\sigma^2_{\kappa, 1}, \dots, \sigma^2_{\kappa, p}$. For the initialization, values of $X_{1 - p}, \dots, X_0$ and $E_{1 - q}, \dots, E_0$ can either be fixed or assigned suitable initialization distributions. % For the compounding distribution $G$ we assume again finite mean $\mu_\psi$ and variance $\sigma^2_\psi$. 
...
In the special case $p = q = 1$ the Poisson INARMA(1, 1) model can be written in a quite compact fashion without the need for the auxiliary processes $\{A_t\}, \{L_t\}$ and $\{C_t\}$.% (see Supplement \ref{XYZ} for the short derivation):
\begin{remark}
...
\sigma^2_X & = \frac{\kappa\{1 + \beta\}}{1 + \beta + (1 - \beta)\kappa} \cdot \frac{\mu_I}{1 - \kappa}\label{eq:sigma2_X}  \ \ \ + \ \ \  \left(1 - \frac{\kappa(1 + \beta)}{1 + \beta + (1 - \beta)\kappa}\right) \cdot \frac{\sigma^2_I}{1 - \kappa} \\  % \frac{\sigma^2_I}{1 - \kappa} -
...
As the class of compound Poisson distributions is closed to binomial thinning, the import term $\kappa \circ I_{t - 1}$ in \eqref{eq:E_INAR} follows a compound Poisson distribtion whenever $I_t$ does. In this case it follows that $E_t$ and $X_t$ likewise follow a compound Poisson distribution.\todo{ref to Schweer?} An interesting special case arises for the Hermite distribution, which \cite{Fernandez-Fontelo2017} have treated in the context of the INAR($p$) model. % A random variable $Z$ is said to be Hermite distributed if it can be written as $Z = Y_1 + 2Y_2$ where $Y_1$ and $Y_2$ independently follow Poisson distributions with parameters $a_1$ and $a_2$, respectively. In slight variation of \cite{Gupta1974} we parametrize the distribution by its mean $\tau = a_1 + 2a_2$ and a dispersion parameter\footnote{\cite{Gupta1974} use $d = 1 + \psi$ as the dispersion parameter, which corresponds to the index of dispersion.} $\psi = 2a_2/(a_1 + 2a_2) \in [0, 1]$. Under this parameterization the probability mass function is
% $$
% \text{Pr}(Z = z) = \exp\left[\tau\left(-1 + \frac{\psi}{2}\right)\right] \tau^z(1 - \psi)^z \sum_{j = 0}^{[z/2]} \frac{\psi^j}{2^j\tau^j(1 - \psi)^{2j}(z - 2j)!j!}, z = 0, 1, 2, \dots
% $$
% $$
% \text{Pr}(Z = z) = \exp(-a_1 + a_2) \cdot \sum_{j = 0}^{[z/2]} \frac{a_1^{z - 2j}a_2^j}{(z - 2j)!j!}
% $$
% where $[z/2]$ is the integer part of $z/2$. The mean and variance are given by $\mathbb{E}(Z) = \lambda$ and $\text{Var}(Z) = (1 + \psi)\lambda$. If we now assume that in model \eqref{eq:inarma_poisson_simple_X}--\eqref{eq:inarma_poisson_simple_E}
...
\noalign{\smallskip}\hline\noalign{\smallskip}
\input{table/sim_hermite_sc1.tex}
\noalign{\smallskip}\hline\noalign{\smallskip}
\input{table/sim_hermite_sc2.tex}
\noalign{\smallskip}\hline\noalign{\smallskip}
\input{table/sim_hermite_sc3.tex}
\noalign{\smallskip}\hline\noalign{\bigskip}
...
Overall the fitting procedures work well for all three distributional assumptions and the three scenarios. For $T = 250$ there are some small-sample biases. These largely disappear for $T = 1000$. Little surprisingly, the variability of the estimates decreases in $T$. Overall the estimated standard errors (via the inverse Fisher information) are roughly unbiased, even though in some instances they underestimate the true variability. Confidence intervals and significance tests based on these standard errors may therefore be somewhat liberal. % \textbf{mention importance of trying several starting values as optimization sometimes gets stuck at very small $\psi$; is transformation mentioned anywhere?}
...
\kappa \star (\psi *_G 1) & \text{if } R_{k, t - 1} = 0. % \ \ (\text{ contribution via } \kappa \star (\psi *_G A_{t - 1})).
\label{eq:Z_t_i}
\end{cases}
\end{align} 
Then we consider
