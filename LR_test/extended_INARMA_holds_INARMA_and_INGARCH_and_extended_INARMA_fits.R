...
for (i in sims_to_run) {
  ...
  dat <- unname(as.matrix(read.csv(
    paste0("Simulation_trajectories/sim_extended_INARMA11_s", s, "_lgt_", lgt, "_pois.csv")
  )))
  ...
  if (continue) {
    res <- read.csv(
      paste0("LR_test/Results/extended_INARMA_fits_extended_INARMA_holds_s", s, "_", lgt,
             ".csv")
    )
    res_null <- read.csv(
      paste0("LR_test/Results/INARMA_fits_extended_INARMA_holds_s", s, "_", lgt,
             ".csv")
    )
    res_ingarch <- read.csv(
      paste0("LR_test/Results/INGARCH_fits_extended_INARMA_holds_s", s, "_", lgt,
             ".csv")
    )
 ...
 #   print(paste0("Alternative refitted: ", refit_iter - 1, " times."))
    ...
    if (k %% 10 == 0) {
      ...
      write.csv(res, file = paste0("LR_test/Results/extended_INARMA_fits_extended_INARMA_holds_s", s, "_", lgt, ".csv"),
                row.names = FALSE)
      write.csv(res_null, file = paste0("LR_test/Results/INARMA_fits_extended_INARMA_holds_s", s, "_", lgt, ".csv"),
                row.names = FALSE)
      write.csv(res_null, file = paste0("LR_test/Results/INGARCH_fits_extended_INARMA_holds_s", s, "_", lgt, ".csv"),
                row.names = FALSE)
    }
  }
