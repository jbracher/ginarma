...
for(s in 1:3){ # loop over scenarios
  ...
  for(lgt in c(250, 500, 1000)){
    ...
    scenario_temp[as.character(lgt), "se_psi"] <- sd(exp(pars[, "logit_psi"])/(1 + exp(pars[, "logit_psi"])), na.rm = TRUE)
    ...
    pars_moment <- read.csv(file = paste0("Results/he_estim_moments_", tau, "_", psi, "_", phi, "_",
...
for(s in 1:3){
  ...
  write(
    print(xtable(to_print_ml), only.contents = TRUE,
          ...
          hline.after = NULL),
    file = paste0("../../Draft/table/sim_herm_sc", s, ".tex")
  )
  ...
  write(
    print(xtable(to_print_ml), only.contents = TRUE,
          include.rownames = FALSE, include.colnames = FALSE,
          hline.after = NULL),
    file = paste0("../../Draft/table/sim_herm_sc", s, "_moments.tex")
  )
