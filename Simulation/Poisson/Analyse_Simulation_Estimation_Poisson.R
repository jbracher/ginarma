...
for(s in 1:3){ # loop over scenarios
  ...
  for(lgt in c(250, 500, 1000)){
    ...
    pars <- read.csv(file = paste0("Results/pois_estim_", tau, "_", phi, "_",
    ...
    ses <- read.csv(file = paste0("Results/pois_ses_", tau, "_", phi, "_",
                                  kappa, "_", lgt, ".csv"))[1:n_available_temp, c("log_tau", "logit_phi", "logit_kappa")]
    ses_delta <- NA*ses; colnames(ses_delta) <- c("tau", "phi", "kappa")
    # for tau:
    ...
    pars_moment <- read.csv(file = paste0("Results/pois_estim_moments_", tau, "_", phi, "_",
                                          kappa, "_", lgt, ".csv"))[, c("tau", "phi", "kappa")]
...
for(s in 1:3){
  ...
  write(
    print(xtable(to_print_ml), only.contents = TRUE,
          ...
          hline.after = NULL),
    file = paste0("../../Draft/table/sim_pois_sc", s, ".tex")
  )
  ...
  write(
    print(xtable(to_print_moment), only.contents = TRUE,
          include.rownames = FALSE, include.colnames = FALSE,
          hline.after = NULL),
    file = paste0("../../Draft/table/sim_pois_sc", s, "_moments.tex")
  )
